//* Note: I actually coded this on my own, without following the section of the course. So things may not be 1:1 to what the course does.

type Investment = {
  investmentName?: string;
  startingInvestment: number;
  extraPerYear: number;
  highExpectedReturnRate: number;
  lowExpectedReturnRate: number;
...
type Result = {
  ...
  startOfYearPortfolio: number;
  endOfYearPortfolio: number;
  profit: number;
  profitPercentage: number;
};

function calculator(data: Investment, duration: number): Result[] {
  let results: Result[] = [];
...
function printResults(results: Result[]) {
    let totalProfit: number = 0 //! Here too LMAO
    console.log("-----------------------------------------------------------")
...
//* Note 2: After watching the section of the course, I can confirm it's very not 1:1 but definitely contains all of the same concepts